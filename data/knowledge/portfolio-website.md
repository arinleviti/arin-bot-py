...
## This Chatbot
The chat assistant on this site was built by Arin entirely from scratch in Python. It uses FastAPI as the server framework, ChromaDB as a vector database, and the Groq API — running an open-weight gpt-oss-120b model — for generation. It implements a full RAG pipeline: knowledge base documents are chunked by section, embedded using a local model via onnxruntime, stored in ChromaDB, and retrieved by semantic similarity at query time to ground the LLM's responses. Beyond retrieval, the bot has real agentic capabilities: it automatically reformulates a question and retries with alternate phrasings if the first attempt can't find an answer in the knowledge base, and it can call a tool to send Arin a live email notification (via Resend) when a visitor expresses interest in hiring or contacting him — only once the visitor has actually provided contact info. Conversation history is capped per session, and requests are rate-limited to prevent abuse. The whole system is containerised with Docker and deployed on Google Cloud Run. Arin built this to demonstrate from-scratch Python and AI engineering skills — not inside a no-code platform, but in real, production-deployed code.